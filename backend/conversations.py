...
from helper_func import check_permission, db, get_current_user_id, load_yaml
...
def add_participant(conv_id):
    ...
    """,
        ...
        cur.execute('SELECT 1 FROM "group" WHERE conversation_id = %s', (conv_id,))
        if cur.fetchone() is not None:
            return {
                "message": "This is a group conversation. Please use the group endpoints to add members!",
            }, 403
    ...
    try:
        with db.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("SELECT name FROM conversation WHERE id = %s", (conv_id,))
            conv_data = cur.fetchone()
        conversation_name = (
            conv_data["name"]
            if conv_data and conv_data["name"]
            else f"Conversation #{conv_id}"
        )
...
@conversations_blueprint.route(
    "/<int:conv_id>/participants/<int:user_id>",
    methods=["DELETE"],
)
...
def remove_participant(conv_id, user_id):
    ...
    """,
        ...
        if cur.fetchone() is not None:
            return {
                "message": "This is a group conversation. Please use the group endpoints to remove members!",
            }, 403
...
def delete_conversation(conv_id):
    ...
    """,
        ...
        if cur.fetchone() is not None:
            return {
                "message": "This is a group conversation. Please delete the group itself using group endpoints!",
            }, 403
...
@conversations_blueprint.route(
    "/<int:conv_id>/messages/<int:message_id>",
    methods=["DELETE"],
)
...
def delete_message(conv_id, message_id):
    ...
    if not can_delete:
        with db.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                'SELECT id_group FROM "group" WHERE conversation_id = %s',
                (conv_id,),
            )
            group_row = cur.fetchone()
        if group_row and check_permission(
            current_user_id,
            group_row["id_group"],
            "delete_messages",
        ):
